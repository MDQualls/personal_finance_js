...
**Status:** Complete — started 2026-07-19, closed out 2026-07-25
...
- [x] Initial sync run by the user — succeeded, balances updated, transactions landed in the review queue as expected. User worked through the review queue over several sessions (approve/edit/recategorize) — **confirmed complete 2026-07-25: 0 `needsReview` transactions remain.**
- [x] Refine `lib/plaidCategories.ts` against real transaction categories — added `BANK_FEES: 'Bank Fee'` (2026-07-25). Real `WIRE FEE` transactions had been landing in Uncategorized and needed manual recategorization every sync — a genuine repeated pattern with an exact matching local category. Checked for similar gaps against the other three deliberately-omitted primaries (`LOAN_PAYMENTS`, `GENERAL_SERVICES`, `GOVERNMENT_AND_NON_PROFIT`): no repeated pattern found for any of them (the one real candidate, a "U.S. Post Office" transaction under `GOVERNMENT_AND_NON_PROFIT`, is a single occurrence with no good specific local category — left as Uncategorized, correctly). Note: there's no stored raw `personal_finance_category.primary` on `Transaction`, so this refinement was done by inspecting final (post-review) categorization patterns on real Plaid-synced transactions, not by re-querying Plaid's API — re-querying would incur Production per-call billing and wasn't necessary here.
- [x] Run transfer detection / link transfers via suggestions panel — **done.** 14 `Transfer` rows now exist, covering every real internet-transfer pair between the two owned accounts (8660 ↔ 8690) back through April. This was unblocked by a separate bug fix (see below) — the Link-as-Transfer modal only checked the 50 most-recent transactions with no date scoping, so older Plaid-backfilled pairs were invisible until that was fixed.

**Gap flagged in an earlier session, now closed (2026-07-25):** `PATCH /api/recurring/[id]` didn't enforce the `plaidManaged` guard that `POST /api/recurring` has had since Phase 5. Added the same guard to `PATCH`: fetches the existing rule first, computes the *effective* `autoPost`/`accountId` (falling back to the existing values for any field not present in the PATCH body — important since a PATCH can flip `autoPost` alone, reassign `accountId` alone, or both), and rejects with 422 if the effective combination would auto-post on a Plaid-managed account. 4 new tests added (`app/api/recurring/[id]/route.test.ts`) covering: flipping `autoPost` true on a Plaid-managed account, reassigning an already-autoPost rule onto a Plaid-managed account, allowing the flip on a non-Plaid account, and skipping the account lookup entirely when effective `autoPost` is false (efficiency check).

**Also closed this session, not part of the original Phase 10 checklist but found via the user's own usage of the Edit Transaction popup:**
- The Link-as-Transfer modal (`components/forms/TransactionForm.tsx`) fetched only the 50 most-recent transactions with no date filter, so any transfer pair older than the ~50 most recent transactions was never surfaced as a candidate — a real, current-impact bug once Plaid's historical backfill made "50 most recent" mean almost nothing relative to a transaction from weeks or months ago. Fixed by scoping the fetch with `from`/`to` params bounded to the edited transaction's date ±5 days.
- The category `<select>` in the Transaction/Subscription/Budget/Recurring Rule edit forms silently dropped any parent category from its option list whenever that category had subcategories — so a transaction/rule tagged directly with a parent category (e.g. "Shopping," "Entertainment") rendered a blank category field. Fixed by adding `<optgroup>` support to the shared `Select` component and rebuilding each form's category options as parent+children groups, matching the already-correct main Transactions page filter. Reproduced and verified live in the browser (Chrome extension) before and after the fix on real affected transactions ("Dollar General" → "Shopping", "Community Thrift" → "Thrifting").
- 11 new regression tests added across these two fixes (`TransactionForm.test.tsx`, `SubscriptionForm.test.tsx`, `BudgetForm.test.tsx`, `RecurringRuleForm.test.tsx`). Committed as `642852e`.
...
- [x] Updated `BACKLOG.md` — `[P4-1] Plaid Integration` marked complete and moved to the Done section with a summary; removed from the Priority 4 blocked tier (`P4-2`/`P4-3` remain, unrelated cron-trigger tasks); updated the file's footer audit line to point at P5 next
- [x] Updated `CLAUDE.md` project structure — added `app/api/plaid/`, `app/api/transfers/` (pre-existing gap, also missing), `lib/plaid.ts`, `lib/plaidCategories.ts`, `lib/crypto.ts`, `lib/transferDetection.ts`, a `settings/connections` note, and the four `PLAID_*`/`ENCRYPTION_KEY` env vars; changed the top-of-file `PLAID.md` reference from "do not read until P4-1 is explicitly started" to "read for any Plaid-related work, P4-1 complete"
- [x] Updated auto-memory (`project_status.md`, `next_session.md`) with final P4-1-complete state — see those files for the current pointer

**P4-1 is now fully complete, all 11 phases.** Final state: real Plaid Production connection live, review queue empty, all real inter-account transfers linked, recurring-rule double-post guard closed on both POST and PATCH, category mapping refined against one observed real gap. 495/495 tests passing, `tsc --noEmit` clean. **Next: P5 reporting enhancements (P5-1 through P5-5, specced in `BACKLOG.md`).**
...
**2026-07-25 — Session end: P4-1 fully complete, all 11 phases.** Two sessions happened since the last log entry:

*First (bug-fix session):* user hit two bugs in the Edit Transaction popup — the Link-as-Transfer modal couldn't find an obvious matching transfer pair, and a transaction's category showed blank on load. Root-caused both live in the browser (Chrome extension connected): the transfer modal only ever scanned the 50 most-recent transactions with no date scoping (irrelevant once Plaid's historical backfill made "50 most recent" not include months-old transactions); the category select silently dropped any parent category from its options whenever it had subcategories. Fixed both, added `optgroup` support to the shared `Select` component, applied the same category-select fix to `SubscriptionForm`/`BudgetForm`/`RecurringRuleForm` too since they had the identical bug. 7 regression tests added. Committed `642852e`, 490/490 passing.

*Second (this session):* closed out the three items the user was told to finish before P5, in order:
1. **Recurring-rule guard gap** — added the `plaidManaged` check to `PATCH /api/recurring/[id]`, mirroring the existing `POST` guard but accounting for partial PATCH bodies (effective `autoPost`/`accountId` fall back to the existing rule's values when not present in the request). 4 new tests.
2. **Review pass confirmed done** (0 `needsReview` transactions in the DB) and **transfer detection confirmed done** (14 `Transfer` rows, every real inter-account pair linked — this was actually unblocked by the transfer-modal fix from the prior session, so linking itself required no further action here). **Category refinement**: added `BANK_FEES → 'Bank Fee'` based on a real repeated pattern (`WIRE FEE` transactions needing manual recategorization every sync); checked the other three omitted primaries against real data and found no comparable pattern, so left them alone. 2 test changes.
3. **Phase 11 docs** — this file, `BACKLOG.md` (P4-1 moved to Done), `CLAUDE.md` (project structure + env vars), and auto-memory, all updated.

**2026-08-02 — Post-P4-1 addendum: M1 Finance (brokerage) linked, surfaced a real product-scope limitation.** Unrelated to the P4-1 closeout above — a new account added afterward. Account mapping was already correct (`type: INVESTMENT`, `plaidManaged: true`, Plaid subtype `brokerage`) by the time this was checked, so no code fix needed there. First sync confirmed what a code read predicted: this app's Plaid Link only ever requests `Products.Transactions` (`app/api/plaid/link-token/route.ts`), never `Products.Investments`, so brokerage-type accounts sync **balance only** — `accountsBalanceGet` works regardless of product, but `transactionsSync()` returns no line items for a brokerage account. Confirmed live: the sync updated Net Worth and M1's balance, created zero `Transaction` rows.

This has a real downstream consequence, not just a data gap: since M1 will never produce its own inflow transaction, the existing Transfer feature (`lib/transferDetection.ts`, requires two linked `Transaction` rows) can never link the checking-side "M1" withdrawal to anything on the M1 side. The user's instinct was to reclassify that withdrawal as a transfer — walked through why that's not achievable as-is, and why it wouldn't even be desirable given the Transfer/Savings-Goal-budget interaction (see `BACKLOG.md`'s M1 entry for the full reasoning). Decision: keep it categorized `Savings & Investments`, and added an `AutoRule` (`"M1"` → `Savings & Investments`) so it stops requiring manual recategorization every month.

**If line-item M1 investment activity (dividends, trades) is ever wanted, this needs a real feature addition**: request `Products.Investments` at Link time (for new items — existing items would need Plaid's update-mode re-link to add a product), add `investmentsTransactionsGet`/`investmentsHoldingsGet` calls to the sync route, and build category mapping for investment-specific transaction types. Not started, not currently planned — surfaced here so a future session doesn't have to rediscover the product-scope limitation from scratch.

Final state: 495/495 tests passing, `tsc --noEmit` clean. **P4-1 is done. Next: P5 reporting enhancements (P5-1 through P5-5).**
