// Pure data, deliberately separate from seed.ts (which has top-level script
// side-effects — it runs main() and calls process.exit() on import). Anything
// that needs to reference this data (e.g. tests cross-checking lib/plaidCategories.ts
// against what's actually seeded) must import from here, never from seed.ts itself.
...
export const SYSTEM_CATEGORIES = [
  ...
  // Bank Fee — referenced by lib/plaidCategories.ts's BANK_FEES mapping; must exist
  // in seed data or Plaid-synced bank-fee transactions silently fall back to Uncategorized.
  { name: 'Bank Fee', color: '#94a3b8', icon: 'receipt', isIncome: false, isSystem: true, isSavings: false },
...
export const SUBCATEGORIES: Record<string, { name: string; color: string; icon: string; isIncome: boolean }[]> = {
  'Income': [
